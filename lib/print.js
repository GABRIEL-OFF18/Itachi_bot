...
let _name = await Promise.resolve(conn.getName(m.sender)).catch(() => '')
let sender = '+' + (m.sender || '').replace('@s.whatsapp.net', '') + (_name ? ' ~ ' + _name : '')
let chat = await Promise.resolve(conn.getName(m.chat)).catch(() => m.chat)
let img
...
log = log.replace('@' + user.split`@`[0], chalk.blueBright('@' + await Promise.resolve(conn.getName(user)).catch(() => user)))
}
console.log(m.error != null ? chalk.red(log) : m.isCommand ? chalk.yellow(log) : log)
}
if (m.messageStubParameters) {
const stubLines = await Promise.all(m.messageStubParameters.map(async jid => {
jid = conn.decodeJid(jid)
let name = await Promise.resolve(conn.getName(jid)).catch(() => '')
return chalk.gray('+' + jid.replace('@s.whatsapp.net', '') + (name ? ' ~' + name : ''))
}))
console.log(stubLines.join(', '))
}
